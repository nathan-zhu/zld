 ...
 */
class Capehenrycollegiate_Zhangziyao
{
    const Username = 'zzhang20';
    const Password = 'yaoyao2002831';
    const UID = 30;
    const School = 'capehenrycollegiate';
    const School_Name = 'Cape Henry Collegiate';
    protected $indexUrl;
    protected $loginUrl;
    protected $postData;
    ...
    protected function authLogin()
    {
        ...
        if ($ret) {
            ...
            if ($ret) {
                ...
                //获取GradeLevel(学年和年级)
                $gradeLevelUrl = self::curl(1, $this->gradeLevelUrl);
                //$gradeLevel = json_decode($gradeLevelUrl, true);
                foreach ($gradeLevelUrl as $gL) {
                    if ($gL['CurrentInd']) {
                        //$this->gradeLevel = str_replace(" Grade", "", $gL['GradeLevel']);
                ...
                foreach ($this->retArr as $key => $students) {
                    ...
                    if ($ret) {
                        foreach ($ret as $i => $arr) {
                            //OfferingType 1 is courses, need check grogress page to confirm it.
...
$obj = new Capehenrycollegiate_Zhangziyao();
$obj->run();
