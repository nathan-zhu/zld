...
class Instructure_Alemany {
    const Username = '19jchen@peninsulacatholic.com';
    const Password = '020514cjl';
    ...
    public function __construct() {
        /*
        utf8:✓
        authenticity_token:OFutDaKg91E0Urg1mco6HAh9ea12oBgXDaoaoLMm+ylTa/U1xPSjYlEo/1fpkAJPTjMgxDzXQkNA6DXoxWKXGQ==
        redirect_to_ssl:1
        pseudonym_session[unique_id]:19jchen@peninsulacatholic.com
        pseudonym_session[password]:020514cjl
        pseudonym_session[remember_me]:0
        */
        $this->indexUrl = "https://peninsulacatholic.instructure.com/login/canvas";
        $this->lUrl = "https://auth.alemany.org/adfs/ls/?SAMLRequest=";
        $this->loginUrl = "https://peninsulacatholic.instructure.com/login/canvas";
        //$this->contextUrl = "https://wlhs.myschoolapp.com/api/webapp/context?_".time();
        $this->contextUrl = "https://peninsulacatholic.instructure.com/?login_success=1";
...
//        $this->schoolYear = date("Y").'+-+'.(date("Y")+1);
    ...
    protected function authLogin() {
        ...
        if($ret) {
            ...
            $oNode = $oDom->find('form#login_form input');
            $i = 1;
            foreach($oNode as $oInput) {
                 $value = $oInput->getAttr('value');
                 switch($i) {
                    case 1:
                       $utf8 = $value;
                        break;
                    case 2:
                        ...
                        break;
                    case 3:
                        $redirect_to_ssl = $value;
                        break;
                 }
                 $i++;
            }
            $data = "utf8=%E2%9C%93&authenticity_token=".$authenticity_token."&redirect_to_ssl=1&pseudonym_session%5Bunique_id%5D=". self::Username ."&pseudonym_session%5Bpassword%5D=". self::Password ."&pseudonym_session%5Bremember_me%5D=0";
            //$data = "utf8=".$utf8."&authenticity_token=".$authenticity_token."&redirect_to_ssl=".$redirect_to_ssl."&pseudonym_session[unique_id]=".self::Username."&pseudonym_session[password]=".self::Password."&pseudonym_session[remember_me]=0";
            ...
            //self::html_put_files($login_ret, "datas/1111.html");
            $ret = self::curl(1, $this->contextUrl);
            //self::html_put_files($ret, "datas/2222.html");
            if($ret) {
